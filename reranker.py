#!/usr/bin/env python
# -*- coding: utf-8 -*-
# pip install chromadb numpy modelscope sentence-transformers torch flagembedding
...
from FlagEmbedding import FlagReranker  # 导入重排序器
...
def main():
    ...
    # 初始化重排序器
    print("加载重排序模型...")
    reranker_dir = snapshot_download("BAAI/bge-reranker-v2-m3")
    reranker = FlagReranker(model_name_or_path=reranker_dir, use_fp16=True)
    ...
    for query in queries:
        ...
        # 打印重排序后的结果
        print("\n重排序后的结果:")
        for i, (doc, metadata, distance, rerank_score) in enumerate(reranked_results):
            print(f"\n重排序结果 {i+1}:")
            print(f"  文档: {doc}")
            print(f"  元数据: {metadata}")
            print(f"  原向量距离: {distance:.4f}")
